...
1. Calculate the sample interval (k):
    ...
    <SimpleLatex formula="k=\frac{N}{n}" />
    <SimpleLatex formula="k=\frac{1000}{100}" />
    <SimpleLatex formula="k=10" />
...
3. Select subsequent members.
    ...
    `r`: The random result that will be used as the number to select a random
    sample.
    `13, 23, 33 ...` are choosen for the sample.

    |S|<SimpleLatex formula="(r+k)"/>|r|
    |-----|-----|-----|
    |1st| 3 + 10| 13|
    |2nd| 13 + 10| 23|
    |3rd| 23 + 10| 33|
...
##### a. Convenience
  ...
  <Tabs>
    ...
    <TabItem label="Disadvantages">
      ...
      employee satisfaction vary considerably between these sites. Hence, conducting
